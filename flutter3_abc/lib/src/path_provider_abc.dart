...
class _PathProviderAbcState extends State<PathProviderAbc>
  ...
  List<Widget> buildBodyList(BuildContext context) {
    ...
    if (pathViewModel.pathMap.value == null) {
      pathViewModel.loadPath().whenComplete(
        () => context.buildContext?.tryUpdateState(),
      );
      ...
      list.add(
        [
          GradientButton(
            onTap: () async {
              setFiles(await pickFiles(allowMultiple: false));
            },
          ...
          GradientButton(
            onTap: () async {
              setFiles(
                await pickFiles(dialogTitle: "选择文件(多选)", allowMultiple: true),
              );
            },

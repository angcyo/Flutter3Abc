...
class _WebviewAbcState extends State<WebviewAbc>
  ...
  List<Widget>? buildAppBarActions(BuildContext context) {
    ...
    return [
      foldLive.buildFn(() {
        //debugger();
        //l.d("foldLive.value:${foldLive.value}");
        if (foldLive.value == true) {
          return loadAbcSvgWidget(
            Assets.svg.unfold,
            size: 24,
            tintColor: globalTheme.appBarForegroundColor,
          ).icon(() {
            foldLive << false;
            updateState();
          });
  ...
  WidgetList buildBodyList(BuildContext context) {
    ...
    return [
      ...
      if (foldLive.value != true) _buildAbcButtonLayout(context),
      if (result != null)
  ...
  Widget _buildAbcButtonLayout(BuildContext context) {
    return [
